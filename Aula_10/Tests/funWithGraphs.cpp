...
int FunWithGraphs::game(const vector<int>& v) {
    MinHeap<int, int> s = MinHeap<int, int>(v.size(),0);
    for (int i = 0; i < v.size(); i++){
        s.insert(v[i], sumDigits(v[i]));
    }
    while(s.getSize() != 1){
        int keyA = s.removeMin();
        int keyB = s.removeMin();
        int newKey = abs(keyA-keyB);
        s.insert(newKey, sumDigits(newKey));
    }
    return s.removeMin();
}

int FunWithGraphs::sumDigits(int n){
    int sum = 0;
    while (n != 0) {
        sum = sum + n % 10;
        n = n / 10;
